...
cp -R checkpoint/2X/g_rtvsrgan_2x checkpoint/2X/generic/ 

echo "--model g_rtvsrgan game"
...
cp -R checkpoint/2X/g_rtvsrgan_2x checkpoint/2X/game/ 

echo "--model percsr g_rtvsrgan game"
...
python generate_model_to_pb.py --model g_rtvsrgan --cluster game --output_folder ../ffmpeg-tensorflow/dnn_models/2X/ --ckpt_path ./checkpoint/2X/game/percsr_2x/g_rtvsrgan/generator/ --scale_factor 2
